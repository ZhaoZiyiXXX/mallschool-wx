...
class MyWechat extends Wechat {
     ...
     */
      protected function onSubscribe() {
          ...
          $items = array(
              new NewsResponseItem('欢迎关注喵校园，专业的二手书交易平台', '', ROOT_URL.'source/logo.png', ''),
              new NewsResponseItem('【买家指南】', '', ROOT_URL.'source/sellbook.png', ROOT_URL.'pages/buyer_readme.php'),
              new NewsResponseItem('【卖家指南】', '', ROOT_URL.'source/buybook.png', ROOT_URL.'pages/seller_readme.php'),
          );
     ...
     */
      ...
      private function hasEnoughInfo() {
          $url=DB_API_URL . "wuser?openid=" . $this->getOpenid();
          $html = file_get_contents($url);
          $obj = json_decode($html);
          if (empty($obj->{'data'}->{'college'}) || empty($obj->{'data'}->{'campus'})) {
              $url='<a href='.ROOT_URL.'pages/userinfo.php?openid=' . $this->getOpenid() . '">请先点击补全校区信息</a>';
              $this->responseText($url);
              return false;
          }
          if (empty($obj->{'data'}->{'tel'})) {
              $url='<a href='.ROOT_URL.'pages/userinfo.php?openid=' . $this->getOpenid() . '">请先点击补全联系信息</a>';
              $this->responseText($url);
              return false;
          }
          return true;
     ...
     */
      protected function onScancodeWaitMsg() {
          ...
          if("EAN_13"==$tmparray[0]){
              $url=DB_API_URL . 'book?type=ISBN&q=' . $tmparray[1];
              $html = file_get_contents($url);
              $obj = json_decode($html);
              if('{"result":0,"data":{"result":0}}' == $html){
                  $url = "亲爱的同学：\n 您要出售的这本书有些特别，可否帮我我们补全一下图书信息？/::*\n <a href=\"" . ROOT_URL ."pages/newbook.php\">点击此处提交图书信息</a>";
                  $this->responseText($url);
              }
              else{
                  $tmpresponse = "亲爱的同学/::) \n您即将出售一本《" . $obj->{"data"}->{"name"} . "》 \n <a href=\"".ROOT_URL."pages/sellbook.php?isbn=" . $tmparray[1] . 
                      "&name="   . $obj->{"data"}->{"name"} . 
              ...
              $url=DB_API_URL . 'book?type=ISBN&q=' . $tmparray[0];
              $html = file_get_contents($url);
              $obj = json_decode($html);
              if('{"result":0,"data":{"result":0}}' == $html){
                  $url = "亲爱的同学：\n 您要出售的这本书有些特别，可否帮我我们补全一下图书信息？/::*\n <a href='".ROOT_URL."pages/newbook.php'>点击此处提交图书信息</a>";
                  ...
                  $tmpresponse = "亲爱的同学/::) \n您即将出售一本《" . $obj->{"data"}->{"name"} . "》 \n <a href=\"".ROOT_URL."pages/sellbook.php?isbn=" . $tmparray[0] . 
                      "&name="   . $obj->{"data"}->{"name"} . 
                      "&author=" . $obj->{"data"}->{"author"} . 
                      "&press="  . $obj->{"data"}->{"press"} .
                      "&price="  . $obj->{"data"}->{"fixedPrice"} . 
          ...
          if ($event_key == 'USER_INFO') {
              ...
              if('{"result":0,"data":{"result":0}}' == $html){
                  ...
                  $this->responseText("个人信息：\n" . 
                                      ...
                                      "【校区】：" . $obj->{"data"}->{"campus"} . "\n" . 
                                      "【电话】：" . $obj->{"data"}->{"tel"} . "\n" .
                                      '<a href="'. ROOT_URL . 'pages/userinfo.php?openid=' . $this->getOpenid() . '">点击修改个人信息</a>');
              }
          } else if ($event_key == 'MY_BOOK_INFO') {
              $url=DB_API_URL . 'wsells?openid=' . $this->getOpenid();
              $html = file_get_contents($url);
              $obj = json_decode($html);
              if($obj->{"result"} == "1000"){
                  $this->responseText("抱歉，获取个人信息异常，请稍后重试。");
                  ...
                  $this->responseText("您当前的在售图书信息：\n" . 
                                      $book_info .
                                      '<a href="'.ROOT_URL.'pages/mysellinfo.php?openid=' . $this->getOpenid() . '">点击查看详细信息</a>');
     ...
     */
      protected function onImage() {
          $this->responseText("同学，发的是你的靓照么？那么我给你讲个笑话吧\n/::D" . $this->get_jock());
      }
     ...
     */
      protected function onLocation() {
          $this->responseText("同学，您当前的位置为：" . $this->getRequest('location_x') . ',' . $this->getRequest('location_y'));
      }
     ...
     */
      ...
      private function runDefaultTest($command) {
          ...
          //$items = array(
          //new NewsResponseItem('标题一', '描述一', ROOT_URL . "source/smalllogo.png", ROOT_URL . "pages/mysellinfo.php"),
          //new NewsResponseItem('标题二', '描述二', ROOT_URL . "source/smalllogo.png", $this->getRequest('picurl')),
          //);
          //$this->responseNews($items);
          ...
          // jqmobile网页测试
          $this->responseText("<a href=\"".ROOT_URL."jqmobile_test/userinfo.php?openid=" . $this->getOpenid() . "\">测试链接</a>");
